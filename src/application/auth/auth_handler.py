...
class AuthHandler:
    ...
    def login(self):
        """Prompts the user for credentials and creates the user if they don't exist."""
        print("--- Chess Login ---")
        ...
        if user_data:
            ...
            self.db.save_player_record(username, 1200, password=password)
            print(f"Account created for {username} with 1200 ELO.")
            return {"username": username, "elo": 1200}
        return None