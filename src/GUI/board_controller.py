...
TEXT_THICKNESS = 2
BOARD_MAX_INDEX = constants.BOARD_SIZE - 1


def show_gui_home_screen() -> tuple[str, str, str, str]:
    root = tk.Tk()
    root.title("Chess Room Portal")
    root.geometry(f"{PORTAL_WINDOW_WIDTH}x{PORTAL_WINDOW_HEIGHT}")
    ...
    user_entry = tk.Entry(root, width=DEFAULT_ENTRY_WIDTH)
    user_entry.pack(pady=3)
    user_entry.insert(0, "Player1")

    tk.Label(root, text="Room Name:").pack()
    room_entry = tk.Entry(root, width=DEFAULT_ENTRY_WIDTH)
    room_entry.pack(pady=3)

    tk.Label(root, text="Password (Optional/Required):").pack()
    pass_entry = tk.Entry(root, width=DEFAULT_ENTRY_WIDTH, show="*")
    ...
    tk.Button(btn_frame, text="Create", width=DEFAULT_BUTTON_WIDTH, command=on_create).grid(row=0, column=0, padx=4)
    tk.Button(btn_frame, text="Join", width=DEFAULT_BUTTON_WIDTH, command=on_join).grid(row=0, column=1, padx=4)
    tk.Button(btn_frame, text="Cancel", width=DEFAULT_BUTTON_WIDTH, command=on_cancel).grid(row=0, column=2, padx=4)
...
class BoardController(Observer):
    ...
    def handle_opponent_disconnection(self):
        if self.disconnect_countdown is None:
            self.disconnect_countdown = DEFAULT_DISCONNECT_COUNTDOWN
            self._disconnect_timer_ms = 0
    ...
    def handle_click(self, row, col):
        ...
        if self.player_color == 'black':
            row, col = BOARD_MAX_INDEX - row, BOARD_MAX_INDEX - col
        ...
        if base_path.exists():
            for folder in base_path.iterdir():
                if folder.is_dir() and (path := folder / "states" / "idle").exists():
                    ...
                    size = int(self.facade.mapper.cell_size * PIECE_ANIMATION_SCALE)
    ...
    async def run_async(self):
        ...
        while True:
            ...
            if self.disconnect_countdown is not None and self.disconnect_countdown > 0:
                ...
                if self._disconnect_timer_ms >= ONE_SECOND_MS:
                    self._disconnect_timer_ms -= ONE_SECOND_MS
                ...
                if self.disconnect_countdown <= 0:
                    ...
                    self.facade._runner.status.game_over = True
                    self.facade._runner.status.winner = self.player_color
